...
while True:
    ...
    if ret_left and ret_right:
        ...
        cv2.drawChessboardCorners(left, CHESSBOARD_SIZE, corners_left, ret_left)
        cv2.drawChessboardCorners(right, CHESSBOARD_SIZE, corners_right, ret_right)
...
ret_left, K1, D1, rvecs1, tvecs1 = cv2.calibrateCamera(
    object_points, image_points_left, gray_left.shape[::-1], None, None
)

ret_right, K2, D2, rvecs2, tvecs2 = cv2.calibrateCamera(
    object_points, image_points_right, gray_right.shape[::-1], None, None
)
...
SAVE_PATH = os.path.join(BASE_DIR, "stereo_calibration.npz")
